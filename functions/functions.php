...
function mvt_register_user() {
    ...
    if (isset($_POST['submitted'])) {
        ...
        if (email_exists($useremail) ) {
            $reg_errors->add('email_exists', esc_html__('Email is not valid', 'mv-translations'));
        }

        if (strlen($password) < 5 ) {
            $reg_errors->add('password_lenght', esc_html__('Password is too short. Please enter at least 5 characters', 'mv-translations'));
        }
